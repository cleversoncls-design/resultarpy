...
export function DesktopRouteShell({ children }: { children: React.ReactNode }) {
  ...
  return <View style={{ backgroundColor: colors.background }} className="flex-1 flex-row">
    <View style={{ borderRightColor: colors.border, backgroundColor: colors.surface }} className="w-[252px] border-r px-5 py-6">
      ...
      {canAdmin ? <><Pressable onPress={() => router.push('/admin-users' as never)} onHoverIn={() => setHoveredPath('/admin-users-direct')} onHoverOut={() => setHoveredPath(null)} style={({ pressed }) => [{ backgroundColor: pathname.startsWith('/admin-users') ? colors.primary : hoveredPath === '/admin-users-direct' ? `${colors.primary}12` : 'transparent', borderRadius: 10, flexDirection: 'row', alignItems: 'center', minHeight: 44, paddingHorizontal: 12, paddingVertical: 10, marginTop: 8, marginBottom: 4, opacity: pressed ? 0.72 : 1 }]}><IconSymbol name="person.crop.circle.fill" size={19} color={pathname.startsWith('/admin-users') ? 'white' : hoveredPath === '/admin-users-direct' ? colors.primary : colors.muted} /><Text style={{ color: pathname.startsWith('/admin-users') ? 'white' : hoveredPath === '/admin-users-direct' ? colors.primary : colors.foreground, marginLeft: 12, fontSize: 14, fontWeight: '700', flex: 1 }}>{t('Usuários locais')}</Text></Pressable></> : null}
