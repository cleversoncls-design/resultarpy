...
export function DemoRoleProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { const queryRole = typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('perfil') : null; const fromQuery = queryRole === 'viajante' ? 'Viajante' : queryRole === 'aprovador' ? 'Aprovador' : queryRole === 'administrativo' ? 'Administrativo' : null; if (fromQuery) { setRoleState(fromQuery); AsyncStorage.setItem(ROLE_KEY, fromQuery); return; } AsyncStorage.getItem(ROLE_KEY).then((saved) => { if (saved === 'Viajante' || saved === 'Aprovador' || saved === 'Administrativo') setRoleState(saved); }); }, []);
