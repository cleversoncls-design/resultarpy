...
function CatalogChoice({ value, options, emptyLabel, onChange }: { value: number | string; options: { id: number | string; label: string }[]; emptyLabel: string; onChange: (value: number | string) => void }) {
  const colors = useColors();
  const { t } = useLanguage();
  ...
  return (
    <>
      <Pressable
        onPress={() => { if (options.length) setOpen(true); }}
        style={({ pressed }) => ({
          borderColor: colors.border,
          backgroundColor: colors.surface,
          opacity: pressed && options.length ? 0.7 : 1,
        })}
        className="flex-row items-center justify-between rounded-2xl border px-4 py-4"
      >
        <Text className={selected ? 'text-foreground' : 'text-muted'} numberOfLines={1} style={{ flex: 1, marginRight: 8 }}>
          {selected ? selected.label : options.length ? t('Selecionar...') : emptyLabel}
        </Text>
        {options.length ? <Text style={{ color: colors.muted }}>▾</Text> : null}
      </Pressable>
      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
        <View className="flex-1 items-center justify-center bg-black/40 px-5">
          <View style={{ maxHeight: '70%' }} className="w-full max-w-md rounded-2xl bg-background p-3">
            <ScrollView showsVerticalScrollIndicator={false}>
              {options.map((option) => (
                <Pressable
                  ...
                  style={({ pressed }) => ({
                    backgroundColor: value === option.id ? `${colors.primary}18` : pressed ? colors.surface : 'transparent',
                  })}
                  className="rounded-xl px-4 py-3"
                >
                  <Text style={{ color: value === option.id ? colors.primary : colors.foreground }} className="text-sm font-semibold">
                    {option.label}
                  </Text>
                </Pressable>
              ))}
            </ScrollView>
            <Pressable onPress={() => setOpen(false)} style={({ pressed }) => ({ opacity: pressed ? 0.65 : 1 })} className="mt-2 items-center rounded-xl border border-border p-3">
