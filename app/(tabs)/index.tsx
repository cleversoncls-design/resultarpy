...
export default function HomeScreen() {
  ...
  return <ScreenContainer className="px-5 pt-5"><View className="w-full max-w-6xl flex-1 self-center"><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
    <View className="mb-6 flex-row items-start justify-between"><View><Text className="text-sm text-muted">{t('Bom dia,')}</Text><Text className="mt-1 text-3xl font-bold text-foreground">{demoUser.name}.</Text><Text className="mt-1 text-sm text-muted">{t(`Perfil: ${role}`)}</Text></View><View style={{ backgroundColor: colors.primary }} className="h-11 w-11 items-center justify-center rounded-xl"><IconSymbol name="airplane" size={22} color="white" /></View></View>
    <View className={isNarrow ? 'mb-6 flex-col gap-3' : 'mb-8 flex-row gap-3'}><MetricCard label={t('Total de viagens')} value="02" /><MetricCard label={t('Aguardando aprovação')} value="02" accent={colors.warning} /><MetricCard label={t('Em preparação')} value={isAdmin ? String(adminQueue.length) : '00'} accent={colors.primary} /><MetricCard label={t('Fechamento enviado')} value={isTraveler ? '00' : formatCurrency(0)} accent={colors.success} /></View>
    ...
    {!isTraveler ? <View className="mt-8 rounded-2xl border border-border bg-surface p-5"><Text className="text-xs font-bold uppercase tracking-widest text-muted">{t('Pendências do ambiente')}</Text><Text className="mt-2 text-base font-bold text-foreground">{approvalQueue.length} {t('aprovações aguardando')}</Text><Text className="mt-1 text-sm text-muted">{t('Solicitações da sua equipe e preparações administrativas.')}</Text></View> : null}
