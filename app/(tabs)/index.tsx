...
export default function HomeScreen() {
  ...
  return <ScreenContainer className="px-5 pt-5"><View className="w-full max-w-6xl flex-1 self-center"><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
    ...
    <SectionHeader title="Próxima atividade" />
    <Pressable onPress={() => router.push('/trip-detail')} style={({ pressed }) => [{ borderColor: colors.border, opacity: pressed ? 0.75 : 1 }]} className="mb-8 rounded-2xl border bg-surface p-5"><View className="flex-row items-start justify-between"><View className="flex-1"><Text className="text-xs font-bold uppercase tracking-widest text-muted">{activeTrip.status}</Text><Text className="mt-2 text-xl font-bold text-foreground">{activeTrip.destination}</Text><Text className="mt-1 text-sm text-muted">{activeTrip.startDate} — {activeTrip.endDate} · {activeTrip.client}</Text></View><StatusPill status={activeTrip.status} /></View><View className="mt-5 flex-row items-end justify-between"><View><Text className="text-xs text-muted">Adiantamento</Text><Text className="mt-1 text-base font-bold text-foreground">{formatCurrency(activeTrip.amount)}</Text></View><Text className="text-sm font-bold text-primary">Ver detalhes ›</Text></View></Pressable>
    <SectionHeader title="Acesso rápido" />
    <View className={isNarrow ? 'flex-col gap-3' : 'flex-row flex-wrap gap-3'}>{visibleActions.map((card) => <Pressable key={card.title} onPress={() => router.push(card.path as never)} style={({ pressed }) => [{ borderColor: colors.border, opacity: pressed ? 0.72 : 1 }]} className={isNarrow ? 'rounded-2xl border bg-surface p-5' : 'w-[48.5%] rounded-2xl border bg-surface p-5'}><View style={{ backgroundColor: `${colors.primary}10` }} className="mb-5 h-10 w-10 items-center justify-center rounded-xl"><IconSymbol name={card.icon} size={20} color={colors.primary} /></View><Text className="text-base font-bold text-foreground">{card.title}</Text><Text className="mt-1 text-sm text-muted">{card.detail}</Text></Pressable>)}</View>
