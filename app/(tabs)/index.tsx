...
export default function HomeScreen() {
  ...
  return <ScreenContainer className="px-5 pt-5"><View className="w-full max-w-6xl flex-1 self-center"><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
    ...
    {isAdmin ? <>
      ...
      <View className={isNarrow ? 'mb-8 flex-col gap-3' : 'mb-8 flex-row gap-3'}><MetricCard label={t('Veículos cadastrados')} value={String(liveVehicles.length).padStart(2, '0')} accent={colors.primary} /><MetricCard label={t('Disponíveis')} value={String(fleetAvailable).padStart(2, '0')} accent={colors.success} /><MetricCard label={t('Alertas de ação')} value={String(fleetAlerts).padStart(2, '0')} accent={colors.warning} /><MetricCard label={t('Em manutenção')} value={String(fleetMaintenance).padStart(2, '0')} accent={colors.error} /></View>
