    ...
    <ScreenContainer className="px-5 pt-4">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 36 }}>
        <View className="mb-6 flex-row items-start justify-between">
          <View className="flex-1 pr-4"><Text className="text-sm font-medium text-muted">Bom dia,</Text><Text className="mt-1 text-3xl font-bold tracking-tight text-foreground">{demoUser.name.split(' ')[0]}.</Text><Text className="mt-1 text-sm text-muted">Seu painel de viagens corporativas</Text></View>
        ...
        <View className="mb-6 rounded-3xl bg-primary p-5">
          ...
          <View className="mt-5 flex-row items-center justify-between"><View><Text className="text-xs text-white/65">Adiantamento</Text><Text className="mt-1 text-base font-bold text-white">{formatCurrency(activeTrip.amount)}</Text></View><Pressable onPress={() => router.push('/trip-detail')} style={({ pressed }) => [{ backgroundColor: 'rgba(255,255,255,0.18)', opacity: pressed ? 0.7 : 1 }]} className="flex-row items-center rounded-xl px-3 py-2"><Text className="mr-1 text-sm font-bold text-white">Ver detalhes</Text><IconSymbol name="chevron.right" size={16} color="white" /></Pressable></View>
        </View>

        <SectionHeader title="Visão geral" />
        <View className="mb-7 flex-row gap-3"><MetricCard label="Em andamento" value="01" accent={colors.primary} /><MetricCard label="Aguardando" value="02" accent={colors.warning} /><MetricCard label="Total no mês" value="R$ 2,8k" accent={colors.success} /></View>

        <SectionHeader title="Ações rápidas" />
        <View className="mb-7 flex-row gap-3"><View className="flex-1"><PrimaryButton label="Nova viagem" onPress={() => router.push('/new-trip')} /></View><View className="flex-1"><Pressable onPress={() => router.push('/expenses')} style={({ pressed }) => [{ borderColor: colors.border, opacity: pressed ? 0.7 : 1 }]} className="flex-row items-center justify-center rounded-2xl border bg-surface px-4 py-4"><IconSymbol name="wallet.pass.fill" size={18} color={colors.primary} /><Text className="ml-2 font-bold text-foreground">Lançar despesa</Text></Pressable></View></View>

        <SectionHeader title="Pendências do ambiente" action="Ver tudo" />
